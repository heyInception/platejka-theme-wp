        ...
        <div class="calc__row">
          <div class="calc__item">
            ...
            <div class="calc__input">
              ...
              <div class="calc__input_input">
                <input type="number" id="amount" oninput="this.value=this.value.replace(/[^0-9]\./g,'');" placeholder="Введите сумму перевода">
          ...
          <div class="calc__item calc__item_green">
            ...
            <div class="calc__wrapper">
              ...
              <button class="btn-reset calc__button_main">Начать перевод в Телеграмм</button>
              <button class="btn-reset calc__button_mail fancybox-inline">Отправить заявку на перевод</button>
              <a href="#fancyboxID-2" class="fancybox-inline calc__button_none"></a>
            </div>
          </div>
        </div>
      </div>
  ...
  <script>
    document.addEventListener("DOMContentLoaded", () => {
      ...
      document.querySelector('.calc__button_main').addEventListener('click', async () => {
        const amountInput = document.getElementById('amount'); // Предполагаем, что поле имеет ID 'amount'
        const amount = parseFloat(amountInput.value);

        // Валидация поля
        if (!amount || isNaN(amount)) {
          // Добавляем красную рамку и прерываем выполнение
          amountInput.style.border = '1px solid red';
          amountInput.style.outline = 'none';

          // Убираем красную рамку через 3 секунды
          setTimeout(() => {
        ...
        // Сбрасываем стиль если валидация прошла
        amountInput.style.border = '';
        ...
        setTimeout(() => {
          ...
          button.textContent = 'Начать перевод в Телеграмм'; // Изменение текста кнопки
          button.disabled = false; // Включение кнопки
        }, 1500);
      });
      document.querySelector('.calc__button_mail').addEventListener('click', async () => {
        const amountInput = document.getElementById('amount'); // Предполагаем, что поле имеет ID 'amount'
        const amount = parseFloat(amountInput.value);
        ...
        amountInput.style.border = '';
        if (!amount || isNaN(amount)) return;
        const button = document.querySelector('.calc__button_mail');
        const countrySelect = document.getElementById('countryTo');
        const selectedCountryText = countrySelect.options[countrySelect.selectedIndex].text;
        ...
        const textarea = document.getElementById('ta911');
        if (textarea) {
        ...
        // Перенаправление в Telegram через 3 секунды
        setTimeout(() => {
          const formLink = document.querySelector('a[href="#fancyboxID-2"]');
          if (formLink) {
            formLink.click();
          }
          button.textContent = 'Отправить заявку на перевод';
    ...
    // Функция для расчета комиссии по новым правилам
    function calculateCommission(amount) {
      if (amount >= 7500) {
        // 0.1% от суммы если 7500 и выше
        return amount * 0.001; // 0.1% = 0.001
      } else {
        // Фиксированная комиссия 275 для сумм до 7500
        return 275;
      }
    }

    async function calcEurToRub(x) {
      // Используем новую функцию расчета комиссии
      const com_5 = calculateCommission(x);
      const com = x + com_5;
      x += com_5;
      ...
      return {
        ...
        com95X: parseFloat((x / 100 * 99.9).toFixed(2)),
      };
    }
